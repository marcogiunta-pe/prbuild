...
export function SocialProofTicker() {
  ...
  return (
    <div className="fixed bottom-4 left-4 z-40 max-w-sm animate-in slide-in-from-left duration-300">
      <div className="bg-paper-light/95 backdrop-blur border border-rule rounded-md px-4 py-3 flex items-center gap-3">
        <span className="text-lg">{MESSAGES[current].icon}</span>
        <p className="text-sm text-ink flex-1">{MESSAGES[current].text}</p>
        <button
          onClick={() => setDismissed(true)}
          className="p-1 text-ink-muted hover:text-ink rounded min-h-[44px] min-w-[44px] flex items-center justify-center touch-manipulation"
          aria-label="Dismiss"
        >
          <X className="h-4 w-4" />
        </button>
